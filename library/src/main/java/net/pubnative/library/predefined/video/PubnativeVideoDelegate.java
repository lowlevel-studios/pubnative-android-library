...
//        requestAd();
    ...
    private void requestStaticAd(){
        ...
        new VASTParser(context).setListener(new VASTParser.Listener() {
            ...
            public void onVASTParserError(int error) {
                invokeListenerFailed(new Exception("PubnativeVideoDelegate - ERROR: " + error));
            }
    ...
    private void createPlayer(VASTModel model){
    ...
    @Override
    public void onVideoAdRequestFinished(VideoAdRequest request, List<VASTModel> ads) {

        if (ads.size() > 0) {

            createPlayer(ads.get(0));
